...
parser.add_argument('--testdataroot', type=str, default='/media/lior/LinuxHDD/datasets/MSCeleb-cleaned',#default='/media/lior/LinuxHDD/datasets/vgg_face_dataset/aligned'
                    help='path to dataset')
...
def testaccuracy(test_loader,model,epoch):
    ...
    for batch_idx, (data, label) in pbar:
        ...
        # compute output
        prediction = model.forward_classifier(data_v)
        prec.append(accuracy(prediction.data, label.cuda(), topk=(1,)))
        correct += accuracy(prediction.data, label.cuda(), topk=(1,))*data_v.size(0)
        if batch_idx % args.log_interval == 0:
            pbar.set_description(
                'Test Epoch: {} [{}/{} ({:.0f}%)]\t'
                'Test Prec@1 {:.2f} ({:.2f})\ttotal correct number@1 {:d}'.format(
                    epoch, batch_idx * len(data_v), len(test_loader.dataset),
                    100. * batch_idx / len(test_loader),
                    float(prec[-1]), float(data_v.size(0)),correct)) 
